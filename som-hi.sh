...
for pot in $(seq -- -34 0.1 -36); do
    for hi in $(seq 0.39 0.01 0.43); do
        for lo in $(seq 0.29 0.01 0.33); do
            for r1 in $(seq 0.96 0.02 0.98); do
...
# -35.8 0.40 0.30 0.96    ===>    TOTAL:  92.03 %
...
#     scripts/run_get_pitch.sh -35.8 0.40 0.30 0.96 $clip | grep TOTAL # Ejecuta el Test para los valores actuales y imprime su Fscore total (solo imprimiendo la línea con la palabra "TOTAL")
# done | sort -t: -k 2n # Ordena resultados por Fscores de mayor a menor
...
#   scripts/run_get_pitch.sh -35.8 0.40 0.30 0.96 0.015 3 $harm | grep TOTAL # Ejecuta el Test para los valores actuales y imprime su Fscore total (solo imprimiendo la línea con la palabra "TOTAL")
# done | sort -t: -k 2n # Ordena resultados por Fscores de mayor a menor
